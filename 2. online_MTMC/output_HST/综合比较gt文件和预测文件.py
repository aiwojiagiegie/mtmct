...
if __name__ == '__main__':
    version = opt.version
    # version = '9'
    gt_file = 'D:\研究生实验\Fast_Online_MTMCT\Fast_Online_MTMCT\\2. online_MTMC\output_HST\\test_gt.txt'
    pred_file = f'D:\研究生实验\Fast_Online_MTMCT\Fast_Online_MTMCT\\2. online_MTMC\output_HST/result/version/v{version}.txt'
    baseline_file = 'D:\研究生实验\Fast_Online_MTMCT\Fast_Online_MTMCT\\2. online_MTMC\output_HST/result/version/HST_70.01%.txt'
    ...
    # 创建表格，添加baseline列
    table = PrettyTable()
    table.field_names = ["   GT车辆ID   ", "   预测ID   ", "   baseline ID   ", "  相机ID  ", "  GT帧范围  ", "  预测帧范围  ", "  基准帧范围  "]
                ...
                for camera_id, frame_range in sorted(camera_info.items()):
                    ...
                    if first_pred and first_camera:
                        table.add_row([gt_car_id, pred_car_id, display_baseline_id, camera_id, 
                                     ...
                                     f"{int(baseline_range[0])}-{int(baseline_range[1])}" if baseline_range[0] != -1 else "-"])
                        first_pred = False
                    elif first_camera:
