...
def analyze_vehicle_frames(bbox_data):
    ...
    for camera_id in bbox_data:
        for frame_number in bbox_data[camera_id]:
            for _, _, _, _, car_id in bbox_data[camera_id][frame_number]:
                if car_id not in vehicle_frames:
                    vehicle_frames[car_id] = {}
                if camera_id not in vehicle_frames[car_id]:
                    vehicle_frames[car_id][camera_id] = {'start': frame_number, 'end': frame_number}
                else:
                    vehicle_frames[car_id][camera_id]['end'] = frame_number

    result = []
    for car_id, camera_data in vehicle_frames.items():
        for camera_id, frame_info in camera_data.items():
...
if __name__ == '__main__':
    ...
    # 分析每个车辆在每个摄像头中的帧情况
    vehicle_frame_analysis = analyze_vehicle_frames(bbox_data)
    
    print("\n每个车辆在每个摄像头中的起止帧情况及帧差值:")
    vehicle_frame_table = PrettyTable()
    vehicle_frame_table.field_names = ["车辆ID", "摄像头ID", "起始帧", "结束帧", "帧差值"]
