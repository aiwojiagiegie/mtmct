...
def read_gt_file(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            camera_id,  track_id, frame, left, top, width, height, _, _ = map(float, line.strip().split(' '))
...
def process_dataset(frames, dataset_type):
    for frame, filename, frame_annotations in tqdm(frames, desc=f"处理{dataset_type}集"):
        ...
        for ann in frame_annotations:
            bbox_coco = convert_to_coco(ann['bbox'], img_width, img_height)
